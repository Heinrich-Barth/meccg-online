...
class GameEvents
{
    ...
    pallandoInPlay = false;
    pallandoIsMine = false;
    pallandoOwner = "";
    eventCodes:{ [key:string] : Function[]} = { };
    genericEvents:{ [key:string] : Function} = { };
    myId = g_sUserId;
    isWatcher = document.body.getAttribute("data-is-watcher") === "true";

    registerGenericEvent(eventId:string, fnCallback:Function)
    {
     ...
     */
    onPlayFromHand(bIsMe:boolean, data:any)
    {
    ...
    removeEmptyCardDivs()
    {
        ...
        const list:any = table.querySelectorAll("div.card");
        if (list.length === 0)
            return;
    ...
    markNonPermanentEvents()
    {
        ...
        this.forEachCardIn(this.#getCardsInStagingAreaLongShort(), function(card:any) 
        {
    ...
    static getAttributeInteger(val:any)
    {
        try
        {
            if (val && val !== "")
    ...
    autoFlip()
    {
        ...
        pArea.childNodes.forEach((card:any) => {
    ...
    #getCardsInStagingAreaLongShort()
    {
        ...
        for (let elem of this.#getCardsInStagingArea(true) as any)
            list.push(elem);

        for (let elem of this.#getCardsInStagingArea(false) as any)
            list.push(elem);

        return list;
    }

    forEachCardIn(list:any, fnCall:Function)
    {
    ...
    onScoredShared(_bIsMe:boolean, data:any)
    {
     ...
     */
    onMoveToPile(bIsMe:boolean, data:any)
    {
    ...
    registerEventCode(code:string, ...callbacks:Function[])
    {
        this.eventCodes[code] = callbacks;
    }

    triggerEvent(code:string, isMe:boolean, type:number, data:any)
    {
        try
        {
    ...
    triggerGenericEvent(eventId:string, data:any)
    {
        try
        {
    ...
    onArdaEventWizardPlayed(isMe:boolean, type:number, data:any)
    {
    ...
    onEventPallando(isMe:boolean, type:number, data:any)
    {
...
export function RegisterGenericEvent(eventId:string, fnCallback:Function)
{
    GameEvents.INSTANCE.registerGenericEvent(eventId, fnCallback);
}

export default function InitGameEvents()
{
