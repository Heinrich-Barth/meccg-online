...
export interface ArdaOnTradeSuccess {
    getOpeningHands():void;
    getRegularHand():void;
}

export default class ArdaExchangeBox extends TradeCards {

    #callbackTradeSuccess:ArdaOnTradeSuccess;

    constructor(callbackTradeSuccess:ArdaOnTradeSuccess)
    {
        super();
        
        this.#callbackTradeSuccess = callbackTradeSuccess;
    }
    ...
    tradeSuccess(isMe:boolean, jData:any)
    {
        if (super.tradeSuccess(isMe, jData))
        {
            this.#callbackTradeSuccess.getOpeningHands();
            this.#callbackTradeSuccess.getRegularHand();
        }

        return true;
    }
    
    create(leftIconDivId:string)
    {
