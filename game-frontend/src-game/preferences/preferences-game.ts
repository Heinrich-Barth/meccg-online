...
export class GamePreferences extends Preferences {
    ...
    setBackgroundImage(sNew:string)
    {
    ...
    #toggleSmallPreview(isActive:boolean)
    {
        if (isActive)
    ...
    #toggleStackStage(isActive:boolean)
    {
        this.#toggleClass(document.querySelector(".table"), isActive, "table-stage-stacking");
    }

    #toggleClass(elem:any, isActive:boolean, className:string)
    {
    ...
    #doubleMiscPoints(isActive:boolean)
    {
        MeccgApi.send("/game/score/doublemisc", { misc: isActive === true });
    }

    #toggleCompanyHoverBackground(isActive:boolean)
    {
        if (isActive)
            sessionStorage.setItem("toggle_white", "yes");
        else if (sessionStorage.getItem("toggle_white"))
    ...
    #toggleSpanishCards(isActive:boolean)
    {
    ...
    #toogleCompanyLineBreak(isActive:boolean)
    {
    ...
    _replaceImageLanguage(img:any, useEnglish:boolean)
    {
    ...
    #replaceImageLanguageAttribute(img:any, useEnglish:boolean, attribName:string)
    {
    ...
    #backgroundDarkness(isActive:boolean)
    {
    ...
    #insertHelp()
    {
        const elem:any = document.body.querySelector(".help-icon");
        if (elem)
        {
    ...
    #showHelp()
    {
        ...
        new Question("", false).show(
            Dictionary.get("conf_cut_title", "Tips & Shortcuts"), 
            content, 
    ...
    toggleZoom()
    {
        ...
        const level = parseInt(elem.getAttribute("data-level") ?? "0");
...
export default g_pGamesPreferences;