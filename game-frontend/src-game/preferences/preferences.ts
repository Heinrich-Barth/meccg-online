...
export class PreferencesStorable {
    ...
    updateCookie(key:string, value:any)
    {
        this.data[key] = value;
...
export default class Preferences extends PreferencesStorable {

    _htmlCurrentSection:any = null;
    _html:any = [];
    ...
    addConfigToggle(id:string, title:string, initialValue:any, pCallback:Function = Preferences._emptyCallback, type_on = "fa-toggle-on", type_off = "fa-toggle-off")
    {
        if (typeof pCallback === "undefined")
            pCallback = Preferences._emptyCallback;
    ...
    addConfigSlider(id:string, title:string, max:number, initialValue:number, icon:string, pCallback:Function)
    {
        if (typeof pCallback === "undefined")
            pCallback = Preferences._emptyCallback;
    ...
    static onEventClick(e:any)
    {
        const pthis:any = this;
        const id = pthis.getAttribute("data-id");
    ...
    static onEventChangeCheckbox(pThis:any)
    {
    ...
    static onEventChange(e:any)
    {
        ...
        if (typeof Preferences.config[id] !== "undefined")
        {
            if (Preferences.config[id].type === Preferences.Type.CHECKBOX)
                ...
                Preferences.config[id].value = pthis.value;
    ...
    init()
    {
        ...
        ArrayList(document.getElementById("config-panel")).find("input").each((_el:any) => _el.onchange = Preferences.onEventChange);
        ArrayList(document.getElementById("config-panel")).find("label[data-type=action]").each((_el:any) => _el.onclick = Preferences.onEventClick);
    }
}
