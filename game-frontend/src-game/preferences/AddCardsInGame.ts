...
const AddCardsInGame = {
    ...
    removeQuotes : function(sCode:string) 
    {
        if (sCode.indexOf('"') === -1)
            return sCode;
        else
    ...
    getCode : function(line:string)
    {
        return this.removeQuotes(line);
    },

    toDeck : function(sText:string)
    {
        try
        {
            const jDeck = [];
    ...
    createHtml : function()
    {
        ...
        const _div:any = div.querySelector(".config-panel-overlay");
        if (_div)
...
export default function AddCardsToSideboard()
{
    AddCardsInGame.onEvent();
}
