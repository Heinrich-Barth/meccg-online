...
class ChatBox {
    ...
    static getTimeString(lTime:number) {
    ...
    #getDictionaryEntry(key:string)
    {
        return Dictionary.get(key, this.#getDefaultValue(key));
    }

    #createSection(title:string, min:number, maxInc:number) {
        const div = document.createElement("div");
    ...
    #ensureLength(sText:string, nLen:number) {
    ...
    #predefMessage(sFrom:string, id:string) {
    ...
    #textMessage(sFrom:string, sText:string) {
    ...
    #message(sFrom:string, sText:string, id:string, usertext = "", isme = false) {
    ...
    static OnChatMessageReceived(e:any) {
        new ChatBox().#message(
            e.detail.name, 
            e.detail.message, 
            e.detail.id, 
