...
export default class TurnStats {
    ...
    #createDataEntry(id:string, time:number)
    {
    ...
    #printStats()
    {
        ...
        if (target !== null && target.parentElement)
        {
    ...
    toggleStats(e:any)
    {
        ...
        if  (elem === null || table1 === null || table2 === null)
            return false;
        ...
        if (table1.classList.contains("hidden"))
        {
            ...
            elem.innerText = Dictionary.get("turnstats_showdur", "Show turn duration statistics");
        }
        else
        {
            table1.classList.add("hidden");
            table2.classList.remove("hidden");
            elem.innerText = Dictionary.get("turnstats_score", "Show final scoring table");
        }
    ...
    #createToggleLink()
    {
        ...
        a.innerText = Dictionary.get("turnstats_showdur", "Show turn duration statistics");
        a.setAttribute("href", "#");
    ...
    #injectStatsOnFinalScoreValue(id:string, value:string)
    {
    ...
    #isValidTime(t:number)
    {
        return t >= 1000 * 10;
    }

    #sumTurnTime(id:string)
    {
        let sum = 0;
        let turns = 0;
    ...
    #createResultTableEntry(index:number)
    {
        const tr = document.createElement("tr");
    ...
    #createElement(type:string, val:any)
    {
    ...
    #assertLeadingZero(input:number)
    {
        if (input < 10)
            return "0" + input;
        else
    ...
    static insertResults(users:any)
    {
        let bEmpty = true;
        const tBody = document.createElement("tbody");
