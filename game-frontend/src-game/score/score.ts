...
import SavedGameManager from "../savedgame";
...
class ScoringContainers {
    ...
    constructor(props:any, points_min:number, points_max:number)
    {
        this._props = props;
    ...
    createScoreCardTypes()
    {
        ...
        this._props.forEach(function(entry:any)
        {
...
const SCORING_INGAME:any = 
{
    ...
    removeInGame : function(sHexId:string)
    {
        const elem = document.getElementById("scoring-ingame-" + sHexId);
        if (elem === null)
            return;

        const id = elem.getAttribute("data-player-id") ?? "";
    ...
    updateAvatars: function(avatars:any)
    {
    ...
    getFirstCharacter : function(text:string)
    {
    ...
    buildScoreCellMap : function(tr:any)
    {
        const list = tr.getElementsByTagName("td");
        if (list === null || list.length === 0)
            return {};

        const map:{[key:string]:{td:any, value:any, usable:any}} = {};
        for (let td of list)
        {
    ...
    updateCount : function(elem:any, value:number)
    {
    ...
    buildFinalScores_tableRows_cell : function(value:number, usable:any, isCut:boolean, hideZero:boolean)
    {
    ...
    calculateTournamentPoints : function(list:any)
    {
        if (list.length !== 2)
            return [];
    ...
    buildFinalScores_tableFooter : function(map:any)
    {
    ...
    createSimpleNodeElement : function(type:string, value:number, id:string)
    {
    ...
    createListTRPoints:function(list:string[])
    {
        if (list.length !== 2)
    ...
    updateInGameScore : function(tr:any, score:any)
    {
    ...
    updateInGameScoreCutsFinal : function(tr:any, totalUsed:number, total:number)
    {
    ...
    calculateTotals : function(score:any)
    {
        let sum = 0;
        let catcount = 0;
        let highestValue = 0;
    ...
    calculateHalves : function(score:any)
    {
        const result:any = {};
    ...
    updatedStoredData : function(list:any)
    {
    ...
    updateInGameScores : function(list:any)
    {
    ...
    getPlayerTitle(name:string, id:string)
    {
    ...
    getAvatar: function(id:string)
    {
        if (this._avatars[id] === undefined || this._avatars[id] === "")
            return CardList().getBackside();
        else
            return CardList().getImage(this._avatars[id]);
    },

    registerAvatar: function(id:string, code:string, force:boolean)
    {
        if (this._avatars[id] === undefined || force === true)
            this._avatars[id] = code;
    },

    addInGame : function(sName:string, _playerId:string, sHexId:string, isMe:boolean)
    {
    ...
    onUpdateValue : function(link:any, nDif:number)
    {
    ...
    onClickDecrease: function(e:any)
    {
    ...
    onClickIncrease: function(e:any)
    {
        SCORING_INGAME.onUpdateValue(this, 1);
        e.preventDefault();
        return false;
...
const SCORING:any = {
    ...
    ignoreCategory:function(id:string)
    {
        return id !== "" && SCORING.ignore.includes(id);
    },

    _createStatsMap : function(categories:any)
    {
    ...
    _init : function(categories:any, min:number, max:number)
    {
        ...
        if (document.getElementById("scoring-card") !== null)
        {
            ...
            if (elem)
            {
                (elem.querySelector(".menu-overlay") as any).onclick = SCORING.hideScoringCard;
                (elem.querySelector(".button") as any).onclick = SCORING.onStoreCard;
            }
        }

        if (document.getElementById("scoring-sheet") !== null)
        {
            const elem:any = document.getElementById("scoring-sheet");
            if (elem)
            {
            ...
            ArrayList(elem).find("table tbody tr a").each((_elem:any) => _elem.onclick = SCORING.onChangeScoreValue);
        }
    },
    ...
    hideScoringCard : function()
    {
        ...
        ArrayList(document.getElementById("scoring-card")).findByClassName(".score-type").each((_elem:any) => _elem.classList.remove("req"));
        ArrayList(document.getElementById("scoring-card")).findByClassName(".score-points").each((_elem:any) => _elem.classList.remove("req"));
    },
    
    hideScoringSheet : function()
    {
        const sheet = document.getElementById("scoring-sheet");
        if (sheet && !sheet.classList.contains("final-score"))
        {
            sheet.classList.add("hidden");
            ArrayList(sheet).find("span").each((_el:any) => _el.innerText = "0");
        }
    },
    
    displayCard : function(sCode:string)
    {
        const elem = document.getElementById("scoring-card")?.querySelector("img");
        if (elem)
            elem.setAttribute("src", CardList().getImage(sCode));
    },
    ...
    getCurrentSelectValue : function(sName:string)
    {
        let val = "";

        ArrayList(document.getElementById("scoring-card")).find('input[name="' + sName + '"]').each((_el:any) => {
    ...
    sendScoreCard : function(sCardCode:string, isDefault:boolean)
    {
    ...
    scoreCard : function(sCardCode:string)
    {
    ...
    _clickDefault : function(id:string)
    {
    ...
    _getTargetCell(jTable:any, type:string, player:string)
    {
    ...
    scoreSheetUpdateTable(jData:any, jTable:any)
    {
        if (jTable === null)
            return;
        ...
        for (let key in jData)
        {
            ...
            const elem:any = trScoreTotal?.querySelector('th[data-player="'+count.player+'"]');
            if (elem !== null)
                elem.innerText = "" + count.total;
        }
    },

    scoreSheetUpdateTableOnPlayer : function(jTable:any, jData:any, key:string)
    {
        let _elem;
        let total = 0;
    ...
    _showScoreSheet : function(jData:any, bAllowUpdate:boolean)
    {
    ...
    removeUpdateFunctionality()
    {
        ...
        const overlay:any = elem.querySelector(".menu-overlay");
        if (overlay)
        {
    ...
    showScoreSheetCards : function(listCards:any)
    {
    ...
    showScoreSheet : function(jData:any)
    {
        this._showScoreSheet(jData, true, "");
    },
    
    showFinalScore : function(stats:any, automaticShutdown:boolean)
    {
    ...
    showScoreSheetWatch : function(jData:any)
    {
        this._showScoreSheet(jData, false, ""); 
        const elem:any = document.getElementById("scoring-sheet")?.querySelector(".menu-overlay");
        if (elem)
        {
    ...
    _updateStats : function(type:any, points:any)
    {
    ...
    onStoreSheet : function()
    {
        ...
        const sheet = document.getElementById("scoring-sheet");
        ArrayList(sheet).find('td[data-player="self"]').each((elem:any) =>
        {
    ...
    onChangeScoreValue : function(e:any)
    {
    ...
    addPlayers : function(sMyId:string, jMap:any)
    {
    ...
    isAvailable : function(sPlayerId:string, jTable:any)
    {
    ...
    addPlayer : function(sPlayerId:string, sName:string)
    {
        const jTable:any = document.getElementById("scoring-sheet")?.querySelector("table");
        if (jTable === null)
            return;
        ...
        ArrayList(jTable.querySelector("tbody")).find("tr").each(function(_tr:any)
        {
...
const SCORE_API = {
    ...
    addPlayers: function(sMyId:string, jNameMap:any)
    {
    ...
    setDoubleMisc : function(bDouble:boolean)
    {
    ...
    showScoreSheetCards : function(jData:any)
    {
...
export function createScoringApp()
{
    const extractCategories = function(categories:any)
    {
