...
{ 
    ...
    onReorderPlayers(e:any)
    {
        if (this.#reorderHtmlElements(e.detail))
            document.body.dispatchEvent(new CustomEvent("meccg-notify-success", { "detail": Dictionary.get("playerselector_rearranged", "Player seating rearranged.") }));
    }

    #reorderHtmlElements(list:string[])
    {
    ...
    removePlayerIndicator(userid:string)
    {
    ...
    updateHandSize(username:string, nCount:string, nCountPlaydeck:string)
    {
        ...
        if (pContainer !== null)
        {
            let elem:any = pContainer.querySelector(".player-handcard-count");
            if (elem !== null)
    ...
    #setActiveHazardPlayer(playerInfo:{ id:string, isMe:boolean }|null)
    {
        ...
        if (isMe)
            document.body.dispatchEvent(new CustomEvent("meccg-notify-success", { "detail": Dictionary.get("playerselector_hazardplayer", "You are the hazard player.") }));
    }

    #setActivePlayer(sPlayerId:string, bIsMe:boolean)
    {
    ...
    #removeActivePlayer(className:string)
    {
        const list = this.#getActiveElements(className);
        for (let _elem of list as any)
            this.#removeActivePlayerAttribs(_elem, className);
    }

    #getActiveElements(className:string)
    {
        const elem = document.getElementById("player_selector");
        if (elem === null)
            return [];
    ...
    addScoring(sName:string, _playerId:string, sHexId:string, isMe:boolean)
    {
        SCORE_API.addInGame(sName, _playerId, sHexId, isMe);
    }

    #addPlayerGetName(sMyId:string, sName:string, _playerId:string)
    {
        if (_playerId === sMyId)
            return "Myself";
     ...
     * @return {void}
     */
    addPlayers(sMyId:string, jMap:any, playerListOrder:any)
    {
        for (let _playerId in jMap)
        {
            ...
            iCurrent.setAttribute("title", Dictionary.get("playerselector_activeplayer", "Active Player"));
            ...
            iHazard.setAttribute("title", Dictionary.get("playerselector_currenthazard", "Current hazard player"));
            ...
            iHand.setAttribute("title", Dictionary.get("playerselector_cardsinhand", "cards in hand"));
            iHand.innerText = "0";
            ...
            iPlay.setAttribute("title", Dictionary.get("playerselector_cardsindeck", "cards in playdeck"));
            iPlay.innerText = "0";
    ...
    onLoadOpponentView(e:any)
    {
        const pThis:any = this;
        const sHex = pThis.getAttribute("data-hex") || "";
        const jViewContainer = sHex === "" ? null : document.querySelector(".companies[data-player='" + sHex + "']");
        if (jViewContainer !== null)
        {
            ArrayList(document.getElementById("player_selector")).findByClassName("cur").each((el:any) => el.classList.remove("cur"));
            pThis.classList.add("cur");
        
            ArrayList(document.getElementById("opponent_table")).findByClassName("companies").each((el:any) => el.classList.add("hidden"));
    ...
    #setActivePlayerElement(el:any, isPlayer:boolean)
    {
        if (el === null)
            return;
        ...
        if (isPlayer)
        {
            ...
            el.setAttribute("title", Dictionary.get("playerselector_activeplayer", "Active Player"));
        }
        else
        {
            el.classList.add("act-hazard");
            el.setAttribute("data-translate-title", "playerselector_currenthazard");
            el.setAttribute("title", Dictionary.get("playerselector_currenthazard", "Current hazard player"));
        }
    }

    #removeActivePlayerAttribs(elem:any, className:string)
    {
        if (elem === null)
            return;
