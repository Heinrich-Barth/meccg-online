...
const HOOK_URL = process.env.DISCORD ?? "";
const PLATFORM_URL = process.env.PLATFORMURL ?? "";
...
export default class Discord 
{
    ...
    createRoomEntry(room:string) : boolean
    {
    ...
    updateRoomMessageId(room:string, id:string)
    {
        if (this.#roomMessages[room] !== undefined)
            this.#roomMessages[room].id = id;
    }

    updateRoomPlayerList(room:string, name:string)
    {
    ...
    updateRoomMessage(room:string, message:string) : string
    {
        if (this.#roomMessages[room] === undefined)
            return message;
    ...
    saveDiscordMessageId(room:string, data:string)
    {
        try
        {
            if (data !== "")
    ...
    createMessageArda(room:string, name:string, isCreated:boolean, isOpenChallenge:boolean, urlWatch:string, urlJoin:string) : string
    {
        if (!isCreated)
            return `${name} joined.`;
    ...
    creeateMessageStandard(room:string, name:string, isCreated:boolean, isOpenChallenge:boolean, urlWatch:string, urlJoin:string): string
    {
        if (!isCreated)
            return `${name} joined.`;
    ...
    createUrlWatch(room:string, isArda:boolean, platformUrl:string)
    {
        return isArda ? `${platformUrl}/arda/${room}/watch` : `${platformUrl}/play/${room}/watch`;
    }

    createUrlJoin(room:string, isArda:boolean, platformUrl:string)
    {
        return isArda ? `${platformUrl}/arda/${room}` : `${platformUrl}/play/${room}`;
    }

    createDiscordMessage(room:string, isArda:boolean, name:string, isCreated:boolean, isOpenChallenge:boolean): string
    {
    ...
    updateMessage(room:string, hookUrl:string, message:string)
    {
    ...
    getPostOptions(room:string, hookUrl:string) : any
    {
        const options = {
    ...
    doPost(room:string, hookUrl:string, prevMessage:string, processResponse:boolean)
    {
        try 
        {
            const pThis = this;
    ...
    postNotification(room:string, hookUrl:string, message:string, processResponse:boolean)
    {
    ...
    sendDiscordNotificationGame(room:string, isArda:boolean, name:string, isCreated:boolean, isOpenChallenge:boolean = false)
    {
        if (room !== undefined && room !== "")
        {
            if (isCreated)
    ...
    sortScores(score:any, players:any)
    {
    ...
    createScoreMessage(room:string, finalScore:any)
    {
    ...
    createScoredMessageResult(room:string, winner:any, others:any)
    {
    ...
    getEndGameMessage(room:string, gameLog:string)
    {
    ...
    sendDiscordGameEnded(room:string, gameLog:string)
    {
    ...
    calculateFinalScore(name:string, score:any)
    {
        ...
        return {
            name : name ?? "Another player",
            total: total,
            details: message
        } 
    }    

    sendDiscordMessageCreated(room:string, isArda:boolean, name:string)
    {
        this.removeGameEntry(room);
        this.sendDiscordNotificationGame(room, isArda, name, true, false);
    }

    sendDiscordMessageCreatedOpenChallange(room:string, isArda:boolean, name:string)
    {
        this.removeGameEntry(room);
        this.sendDiscordNotificationGame(room, isArda, name, true, true);
    }

    sendDiscordMessageJoin(room:string, isArda:boolean, name:string)
    {
    ...
    registerEvents()
    {
        ...
        {
            addEvent("game-created", this.sendDiscordMessageCreated.bind(this));
            addEvent("game-created-openchallenge", this.sendDiscordMessageCreatedOpenChallange.bind(this))
            addEvent("game-joined", this.sendDiscordMessageJoin.bind(this));
            addEvent("game-finished", this.sendDiscordNotificationFinished.bind(this));
            addEvent("game-remove", this.sendDiscordGameEnded.bind(this));
        }
    }
}
